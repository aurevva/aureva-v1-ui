...
local Library do
    ...
    -- Keep popups embedded in and clipped by their owning scrolling region.
    local function PositionPopup(Popup, RequestedHeight)
        Popup.Position = UDim2New(0, 0, 1, 5)
        Popup.Size = UDim2New(1, 0, 0, MathMax(1, RequestedHeight or 85))
    end

    local function ClampPopupScroll(Scroller)
        local CanvasPosition = Scroller.CanvasPosition
        local MaximumY = MathMax(0, Scroller.AbsoluteCanvasSize.Y - Scroller.AbsoluteSize.Y)
        local ClampedY = MathClamp(CanvasPosition.Y, 0, MaximumY)

        if ClampedY ~= CanvasPosition.Y then
            Scroller.CanvasPosition = Vector2New(CanvasPosition.X, ClampedY)
        end
    end
    ...
    local Components = { } do
        ...
        Components.Dropdown = function(Data)
            ...
            function Dropdown:SetOpen(Bool)
                ...
                Items["OptionHolder"].Instance.Parent = Bool and Items["Dropdown"].Instance or Library.UnusedHolder.Instance
                ...
                if Bool then 
                    ...
                    PositionPopup(Items["OptionHolder"].Instance, Data.MaxSize or 165)
                    ClampPopupScroll(Items["Holder"].Instance)
        ...
        Components.Colorpicker = function(Data)
            ...
            local Items = { } do
                ...
                function Dropdown:SetOpen(Bool)
                    ...
                    Dropdown.IsOpen = Bool
                    DropdownItems["OptionHolder"].Instance.Parent = Bool and DropdownItems["Dropdown"].Instance or Library.UnusedHolder.Instance

                    Debounce = true

                    if Bool then 
                        DropdownItems["OptionHolder"].Instance.Visible = true
                        PositionPopup(DropdownItems["OptionHolder"].Instance, 85)
                    end
        ...
        Components.Keybind = function(Data)
            ...
            Library:Connect(UserInputService.InputBegan, function(Input, GameProcessed)
                local MatchesKey = tostring(Input.KeyCode) == Keybind.Key or tostring(Input.UserInputType) == Keybind.Key
                if not GameProcessed and not Keybind.Picking and Keybind.Value ~= "None" and MatchesKey then
                    if Keybind.Mode == "toggle" then 
                        Keybind:Press()
                    elseif Keybind.Mode == "hold" then 
                        Keybind:Press(true)
    ...
    do -- Element functions
        ...
        Library.ESPPreview = function(self, Params)
            ...
            local IsVisible = Params.Visible ~= false
            local BoundPage = StringLower(tostring(Params.Page or Params.page or Params.Tab or Params.tab or "visuals"))
            local StateProvider = Params.StateProvider or Params.stateProvider
            local ManualPosition = false

            Items["ESPPreview"] = Instances:Create("Frame", {
                Parent = Library.Holder.Instance,
                Name = "\0",
                Position = UDim2New(0, 860, 0, 430),
                Size = UDim2New(0, 258, 0, 334),
                ZIndex = 4,
                BorderSizePixel = 0,
            ...
            Items["ESPPreview"]:Border()

            Items["ESPPreview"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    ManualPosition = true
                end
            end)
            ...
            })  Items["AccentLiner"]:AddToTheme({BackgroundColor3 = "Accent"})
            ...
            Items["Text"] = Instances:Create("TextLabel", {
                ...
                Text = tostring(Params.Name or Params.name or "ESP Preview"),
                Size = UDim2New(0, 0, 0, 15),
            ...
            -- Overlay the common ESP cues on top of the live viewport model.
            Items["ESPBox"] = Instances:Create("Frame", {
                Parent = Items["Background"].Instance,
                Name = "\0",
                Position = UDim2New(0.24, 0, 0.08, 0),
                Size = UDim2New(0.52, 0, 0.86, 0),
                ZIndex = 8,
                BackgroundTransparency = 1,
                BorderSizePixel = 1,
                BorderColor3 = Library.Theme.Accent
            })  Items["ESPBox"]:AddToTheme({BorderColor3 = "Accent"})

            Items["ESPBoxFill"] = Instances:Create("Frame", {
                Parent = Items["Background"].Instance,
                Name = "\0",
                Position = UDim2New(0.24, 0, 0.08, 0),
                Size = UDim2New(0.52, 0, 0.86, 0),
            ...
            })  Items["ESPBoxFill"]:AddToTheme({BackgroundColor3 = "Accent"})

            local BoxSegments = {}
            local function addBoxSegment(Position, Size)
                ...
                table.insert(BoxSegments, Segment.Instance)
            end

            addBoxSegment(UDim2New(0.24, 0, 0.08, 0), UDim2New(0.16, 0, 0, 1))
            addBoxSegment(UDim2New(0.60, 0, 0.08, 0), UDim2New(0.16, 0, 0, 1))
            addBoxSegment(UDim2New(0.24, 0, 0.94, -1), UDim2New(0.16, 0, 0, 1))
            addBoxSegment(UDim2New(0.60, 0, 0.94, -1), UDim2New(0.16, 0, 0, 1))
            addBoxSegment(UDim2New(0.24, 0, 0.08, 0), UDim2New(0, 1, 0.14, 0))
            addBoxSegment(UDim2New(0.76, -1, 0.08, 0), UDim2New(0, 1, 0.14, 0))
            addBoxSegment(UDim2New(0.24, 0, 0.80, 0), UDim2New(0, 1, 0.14, 0))
            addBoxSegment(UDim2New(0.76, -1, 0.80, 0), UDim2New(0, 1, 0.14, 0))

            local ThreeDBoxSegments = {}
            for _ = 1, 12 do
                local Segment = Instances:Create("Frame", {
                    Parent = Items["Background"].Instance,
                    Name = "\0",
                ...
                Segment:AddToTheme({BackgroundColor3 = "Accent"})
                table.insert(ThreeDBoxSegments, Segment.Instance)
            end

            Items["ESPName"] = Instances:Create("TextLabel", {
                Parent = Items["Background"].Instance,
                Name = "\0",
                Position = UDim2New(0, 0, 0.02, 0),
                Size = UDim2New(1, 0, 0, 16),
                ZIndex = 9,
                BackgroundTransparency = 1,
                FontFace = Library.Font,
                TextSize = 11,
                Text = tostring(Params.PlayerName or Params.playerName or LocalPlayer.DisplayName or LocalPlayer.Name),
                TextColor3 = Library.Theme.Accent,
                TextXAlignment = Enum.TextXAlignment.Center,
                BorderSizePixel = 0
            })  Items["ESPName"]:AddToTheme({TextColor3 = "Accent"})

            Items["ESPDistance"] = Instances:Create("TextLabel", {
                Parent = Items["Background"].Instance,
                Name = "\0",
                Position = UDim2New(0, 0, 0.88, 0),
            ...
            Items["ESPWeapon"] = Instances:Create("TextLabel", {
                Parent = Items["Background"].Instance,
                Name = "\0",
                Position = UDim2New(0, 0, 0.94, 0),
                Size = UDim2New(1, 0, 0, 14),
                ZIndex = 9,
                BackgroundTransparency = 1,
                FontFace = Library.Font,
            ...
            Items["ESPHealthText"] = Instances:Create("TextLabel", {
                Parent = Items["Background"].Instance,
                Name = "\0",
                Position = UDim2New(0.22, 0, 0.08, 0),
                Size = UDim2New(0.18, 0, 0, 14),
                ZIndex = 10,
                BackgroundTransparency = 1,
                FontFace = Library.Font,
                TextSize = 10,
                Text = "100 HP",
                TextColor3 = FromRGB(76, 220, 116),
                TextXAlignment = Enum.TextXAlignment.Left,
                BorderSizePixel = 0
            })
            ...
            Items["HealthTrack"] = Instances:Create("Frame", {
                Parent = Items["Background"].Instance,
                Name = "\0",
                Position = UDim2New(0.18, 0, 0.08, 0),
                Size = UDim2New(0, 4, 0.86, 0),
                ZIndex = 9,
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(32, 35, 38)
            })
            ...
            Items["HealthFill"] = Instances:Create("Frame", {
                ...
                ZIndex = 10,
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(76, 220, 116)
            })

            Items["Tracer"] = Instances:Create("Frame", {
                Parent = Items["Background"].Instance,
                Name = "\0",
                AnchorPoint = Vector2New(0.5, 0.5),
                Position = UDim2New(0.5, 0, 0.5, 0),
                ...
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Accent
            })  Items["Tracer"]:AddToTheme({BackgroundColor3 = "Accent"})

            local SkeletonSegments = {}
            local function addSkeletonSegment()
                local Segment = Instances:Create("Frame", {
                    Parent = Items["Background"].Instance,
                    Name = "\0",
                    AnchorPoint = Vector2New(0.5, 0.5),
                ...
                Segment:AddToTheme({BackgroundColor3 = "Accent"})
                table.insert(SkeletonSegments, Segment.Instance)
            end
            for _ = 1, 14 do addSkeletonSegment() end

            local function AlignPreviewToWindow(ForcePosition)
                local MainFrame = Library.MainFrame
                local Frame = Items["ESPPreview"].Instance

                if not MainFrame or not MainFrame.Parent or not Frame.Parent then
                    return
                end
                ...
                local ViewportWidth = CurrentCamera and CurrentCamera.ViewportSize.X or 0
                local RightEdge = Position.X + Size.X
                local AvailableWidth = ViewportWidth - RightEdge - 18
                local PreviewWidth = MathClamp(AvailableWidth, 112, 258)
                local PreviewX = RightEdge + 10

                if ViewportWidth > 0 and PreviewX + PreviewWidth > ViewportWidth - 8 then
                    PreviewX = MathMax(8, ViewportWidth - PreviewWidth - 8)
                end

                if ForcePosition or not ManualPosition then
                    Frame.Position = UDim2New(0, PreviewX, 0, Position.Y)
                end
                Frame.Size = UDim2New(0, PreviewWidth, 0, MathMin(334, Size.Y))
            end

            local function ApplyVisibility(WindowOpen)
                if not Items["ESPPreview"].Instance then
                    return
                end

                local CurrentPage = Library.CurrentPage
                local PageVisible = CurrentPage ~= nil and StringLower(tostring(CurrentPage.Name or "")) == BoundPage
                Items["ESPPreview"].Instance.Visible = IsVisible and PageVisible and WindowOpen ~= false
            end

            AlignPreviewToWindow(true)
            ApplyVisibility(Library.WindowOpenState)
            table.insert(Library.WindowVisibilityCallbacks, ApplyVisibility)

            if Library.MainFrame then
                Library:Connect(Library.MainFrame:GetPropertyChangedSignal("Position"), AlignPreviewToWindow)
                Library:Connect(Library.MainFrame:GetPropertyChangedSignal("Size"), AlignPreviewToWindow)
            end
            ...
            function Preview:SetPage(PageName)
                BoundPage = StringLower(tostring(PageName or "visuals"))
                ApplyVisibility(Library.WindowOpenState)
            end

            function Preview:ResetPosition()
                ManualPosition = false
                AlignPreviewToWindow(true)
            end

            function Preview:SetStateProvider(Provider)
                StateProvider = typeof(Provider) == "function" and Provider or nil
            end
            ...
            local PreviewModel
            local PreviewCenterY = 1.25
            local PreviewDistance = 8
            local PreviewMinX, PreviewMaxX = -2, 2
            local PreviewMinY, PreviewMaxY = -3, 3
            local PreviewHumanoid
            local PreviewHighlight
            local PreviewChamsModel
            local PreviewParts = {}
            local ChamsOverlays = {}
            local SkeletonPairs = {
            ...
            local function ClearViewport()
                ...
                table.clear(ChamsOverlays)
                PreviewHighlight = nil
                PreviewChamsModel = nil
                for _, Segment in SkeletonSegments do
                    Segment.Visible = false
                end
                for _, Object in Items["Viewport"].Instance:GetChildren() do
            ...
            function Preview:RemoveObject(Object)
                ...
                local Overlay = ChamsOverlays[Object]
                ChamsOverlays[Object] = nil
                if Overlay then Overlay:Destroy() end
                if Clone.Parent and Clone.Parent:IsA("Accessory") then
            ...
            function Preview:AddObject(Object)
                ...
                if Object:IsA("BasePart") and Object:FindFirstAncestorWhichIsA("Accessory") then
                    return
                end
            ...
            local function CreateChamsOverlay(Object)
                if not PreviewChamsModel or not Object or not Object:IsA("BasePart") or ChamsOverlays[Object] then
                    return
                end
                ...
                local Success = pcall(function()
                    local Archivable = Object.Archivable
                    Object.Archivable = true
                    Overlay = Object:Clone()
                    Object.Archivable = Archivable
                end)
                if not Success or not Overlay then return end
                ...
                Overlay.Name = "ChamsOverlay"
                Overlay.Parent = PreviewChamsModel
                ChamsOverlays[Object] = Overlay
            end
            ...
            function Preview:BuildFromModel(Model)
                ...
                PreviewHumanoid = Model:FindFirstChildOfClass("Humanoid")
                if PreviewHumanoid then
                    Items["ESPName"].Instance.Text = tostring(Model:GetAttribute("DisplayName") or LocalPlayer.DisplayName or LocalPlayer.Name)
                end
                ...
                local Viewmodel = InstanceNew("Model")
                Viewmodel.Name = "Viewmodel"
                Viewmodel.Parent = WorldModel

                PreviewChamsModel = InstanceNew("Model")
                PreviewChamsModel.Name = "ChamsOverlay"
                PreviewChamsModel.Parent = WorldModel

                PreviewHighlight = InstanceNew("Highlight")
                PreviewHighlight.Name = "ESPChams"
                PreviewHighlight.Adornee = Viewmodel
                ...
                for _, Object in Model:GetDescendants() do
                    ...
                    if Clone then
                        ...
                        if Object:IsA("BasePart") then
                            PreviewParts[Object.Name] = Object
                        end
                    end
                end

                for Object in RenderObjects do CreateChamsOverlay(Object) end

                local Root = Model:FindFirstChild("HumanoidRootPart")
                local RootClone = Root and RenderObjects[Root]
                if RootClone then
                    local RootFrame = RootClone.CFrame
                    for _, Clone in RenderObjects do
                        if Clone and Clone:IsA("BasePart") then
                            Clone.CFrame = RootFrame:ToObjectSpace(Clone.CFrame)
                        end
                    end
                    ...
                    for _, Clone in RenderObjects do
                        if Clone and Clone:IsA("BasePart") then
                            local HalfWidth = Clone.Size.X * 0.5
                            local HalfHeight = Clone.Size.Y * 0.5
                            MinX = MathMin(MinX, Clone.Position.X - HalfWidth)
                            MaxX = MathMax(MaxX, Clone.Position.X + HalfWidth)
                            MinY = MathMin(MinY, Clone.Position.Y - HalfHeight)
                            MaxY = MathMax(MaxY, Clone.Position.Y + HalfHeight)
                        end
                    end
                    if MinX < MaxX and MinY < MaxY then
                        PreviewMinX, PreviewMaxX = MinX, MaxX
                        PreviewMinY, PreviewMaxY = MinY, MaxY
                        PreviewCenterY = (MinY + MaxY) * 0.5
                        PreviewDistance = MathClamp((MaxY - MinY) * 1.34, 7, 9.5)
                    end
                end

                table.insert(ModelConnections, Model.DescendantAdded:Connect(function(Object)
                    local Clone = self:AddObject(Object)
                    if Clone then
                        Clone.Parent = Viewmodel
                        if Object:IsA("BasePart") then
                    ...
                    if Object:IsA("BasePart") then
                        CreateChamsOverlay(Object)
                    elseif Object:IsA("Accessory") then
                        CreateChamsOverlay(Object:FindFirstChild("Handle"))
                    end
                end))

                table.insert(ModelConnections, Model.DescendantRemoving:Connect(function(Object)
                    if Object:IsA("BasePart") and PreviewParts[Object.Name] == Object then
                        PreviewParts[Object.Name] = nil
                    end
            ...
            Library:Connect(RunService.Heartbeat, function()
                ...
                ViewportCamera.CFrame = CFrameNew(
                    Vector3New(0, PreviewCenterY, -PreviewDistance),
                    Vector3New(0, PreviewCenterY, 0)
                )

                local State = {}
                if typeof(StateProvider) == "function" then
                    local Ok, Result = pcall(StateProvider)
                ...
                local ESPEnabled = Settings.Enabled == true
                local function colorOr(Value, Fallback)
                    return typeof(Value) == "Color3" and Value or Fallback
                end

                local function resolveColor(kind, fallback)
                    local rainbowEnabled = Settings.RainbowEnabled == true
                    if rainbowEnabled then
                        if kind == "Box" and Settings.RainbowBoxes == true then return colorOr(Colors.Rainbow, fallback) end
                        if kind == "Skeleton" and Settings.RainbowBoxes == true then return colorOr(Colors.Rainbow, fallback) end
                        if kind == "Tracer" and Settings.RainbowTracers == true then return colorOr(Colors.Rainbow, fallback) end
                        if kind == "Name" and Settings.RainbowText == true then return colorOr(Colors.Rainbow, fallback) end
                    end
                    if Settings.TeamBasedColor == true and State.TeamColor then
                        return colorOr(State.TeamColor, fallback)
                    end
                    if Settings.TeamEsp == true and State.IsTeammate == true then
                        return colorOr(Colors.TeammateColor, fallback)
                    end
                    if Settings.TeamColorEnabled == true then
                        return colorOr(State.IsTeammate and Colors.TeammateColor or Colors.EnemyColor, fallback)
                    end
                    if kind == "Name" and Settings.FriendCheck == true and State.IsFriend == true then
                        return colorOr(Colors.FriendColor, fallback)
                    end
                    return fallback
                end

                local BoxColor = resolveColor("Box", colorOr(Colors.BoxColor, Library.Theme.Accent))
                local NameColor = resolveColor("Name", colorOr(Colors.NameColor, Library.Theme.Accent))
                local TracerColor = resolveColor("Tracer", colorOr(Colors.TracerColor, Library.Theme.Accent))
                local BoxVisible = ESPEnabled and Settings.BoxESP == true
                local BoxStyle = tostring(Settings.BoxStyle or "Corner")
                local BoxOutline = Settings.BoxOutline ~= false
                Items["ESPBox"].Instance.Visible = BoxVisible and BoxOutline and BoxStyle == "Full"
                Items["ESPBox"].Instance.BorderColor3 = BoxColor
                Items["ESPBox"].Instance.BorderSizePixel = MathClamp(MathFloor((tonumber(Settings.BoxThickness) or 1) + 0.5), 1, 3)
                Items["ESPBoxFill"].Instance.Visible = BoxVisible and Settings.BoxFilled == true
                Items["ESPBoxFill"].Instance.BackgroundColor3 = BoxColor
                Items["ESPBoxFill"].Instance.BackgroundTransparency = MathClamp(tonumber(Settings.BoxFillTransparency) or 0.5, 0, 1)
                for _, Segment in BoxSegments do
                    Segment.Visible = BoxVisible and BoxOutline and BoxStyle == "Corner"
                    Segment.BackgroundColor3 = BoxColor
                end
                Items["ESPName"].Instance.Visible = ESPEnabled and Settings.NameESP == true
                Items["ESPName"].Instance.TextColor3 = NameColor
                Items["ESPName"].Instance.TextSize = MathClamp(MathFloor(tonumber(Settings.TextSize) or 11), 8, 18)
                local PlayerLabel = if Settings.NameMode == "Username"
                    then tostring(State.Username or LocalPlayer.Name)
                    else tostring(State.DisplayName or LocalPlayer.DisplayName or LocalPlayer.Name)
                if Settings.FriendCheck == true and State.IsFriend == true then PlayerLabel = "[F] " .. PlayerLabel end
                Items["ESPName"].Instance.Text = PlayerLabel
                Items["ESPDistance"].Instance.Visible = ESPEnabled and Settings.ShowDistance == true
                Items["ESPDistance"].Instance.TextColor3 = colorOr(Colors.Distance, FromRGB(200, 200, 200))
                Items["Tracer"].Instance.Visible = ESPEnabled and Settings.TracerESP == true
                Items["Tracer"].Instance.BackgroundColor3 = TracerColor

                local ChamsEnabled = ESPEnabled and Settings.ChamsEnabled == true
                local ChamsFillColor = resolveColor("Chams", colorOr(Colors.ChamsFillColor, FromRGB(255, 0, 0)))
                local ChamsOutlineColor = resolveColor("ChamsOutline", colorOr(Colors.ChamsOutlineColor, BoxColor))
                local ChamsFillTransparency = tonumber(Settings.ChamsTransparency) or 0.5
                if Settings.ChamsThermal == true then
                    local Pulse = (math.sin(os.clock() * 2) + 1) * 0.5
                    ChamsFillTransparency = MathClamp(ChamsFillTransparency * (0.65 + Pulse * 0.35), 0, 1)
                end
                if PreviewHighlight then
                    PreviewHighlight.Enabled = ChamsEnabled
                    PreviewHighlight.FillColor = ChamsFillColor
                    PreviewHighlight.OutlineColor = ChamsOutlineColor
                    PreviewHighlight.FillTransparency = 1
                    PreviewHighlight.OutlineTransparency = tonumber(Settings.ChamsOutlineTransparency) or 0
                    PreviewHighlight.DepthMode = Settings.ChamsVisibilityCheck and Enum.HighlightDepthMode.Occluded or Enum.HighlightDepthMode.AlwaysOnTop
                end

                local Health = PreviewHumanoid and PreviewHumanoid.Health or 100
                local MaxHealth = PreviewHumanoid and PreviewHumanoid.MaxHealth or 100
                local HealthRatio = MaxHealth > 0 and MathClamp(Health / MaxHealth, 0, 1) or 0
                local HealthColor = FromRGB(255 - (255 * HealthRatio), 255 * HealthRatio, 0)
                local HealthStyle = tostring(Settings.HealthBarStyle or "Bar")
                local ShowHealth = ESPEnabled and Settings.HealthESP == true
                Items["HealthTrack"].Instance.Visible = ShowHealth and (HealthStyle == "Bar" or HealthStyle == "Both")
                Items["HealthTrack"].Instance.Position = UDim2New(Settings.HealthBarSide == "Right" and 0.78 or 0.18, 0, 0.08, 0)
                Items["HealthFill"].Instance.Visible = Items["HealthTrack"].Instance.Visible
                Items["HealthFill"].Instance.Size = UDim2New(1, 0, HealthRatio, 0)
                Items["HealthFill"].Instance.BackgroundColor3 = HealthColor
                Items["ESPHealthText"].Instance.Visible = ShowHealth and (HealthStyle == "Text" or HealthStyle == "Both")
                Items["ESPHealthText"].Instance.Text = string.format("%d %s", math.floor(Health), tostring(Settings.HealthTextSuffix or "HP"))
                Items["ESPHealthText"].Instance.Position = UDim2New(Settings.HealthBarSide == "Right" and 0.80 or 0.22, 0, 0.08, 0)
                Items["ESPHealthText"].Instance.TextColor3 = HealthColor

                local DistanceText = tostring(State.DistanceText or string.format("0 %s", tostring(Settings.DistanceUnit or "studs")))
                Items["ESPDistance"].Instance.Text = DistanceText
                Items["ESPWeapon"].Instance.Visible = ESPEnabled and Settings.WeaponESP == true
                Items["ESPWeapon"].Instance.Text = tostring(State.WeaponName or "None")
                Items["ESPWeapon"].Instance.TextColor3 = colorOr(Colors.WeaponColor, FromRGB(255, 255, 255))

                local SkeletonVisible = ESPEnabled and Settings.SkeletonESP == true
                local SkeletonColor = resolveColor("Skeleton", colorOr(Colors.SkeletonColor, BoxColor))
                local SkeletonThickness = MathClamp(MathFloor((tonumber(Settings.SkeletonThickness) or 1) + 0.5), 1, 3)
                local SkeletonTransparency = 1 - MathClamp(tonumber(Settings.SkeletonTransparency) or 1, 0, 1)
                local function findPart(Candidates)
                    for _, Name in Candidates do
                        if PreviewParts[Name] then return PreviewParts[Name] end
                    end
                end
                local function projectPart(Candidates)
                    local Original = findPart(Candidates)
                    if not Original or not Original.Parent then return nil end
                    local Relative = Root.CFrame:ToObjectSpace(Original.CFrame).Position
                    local BoundsWidth = MathMax(PreviewMaxX - PreviewMinX, 0.01)
                    local BoundsHeight = MathMax(PreviewMaxY - PreviewMinY, 0.01)
                    local BoxFrame = Items["ESPBox"].Instance
                    local Background = Items["Background"].Instance
                    local BoxOrigin = BoxFrame.AbsolutePosition - Background.AbsolutePosition
                    local BoxSize = BoxFrame.AbsoluteSize
                    local X = BoxOrigin.X + MathClamp((Relative.X - PreviewMinX) / BoundsWidth, 0, 1) * BoxSize.X
                    local Y = BoxOrigin.Y + (1 - MathClamp((Relative.Y - PreviewMinY) / BoundsHeight, 0, 1)) * BoxSize.Y
                    return Vector2New(X, Y)
                end
                local function drawSkeletonLine(Line, From, To)
                    ...
                    Line.Position = UDim2New(0, (From.X + To.X) * 0.5, 0, (From.Y + To.Y) * 0.5)
                    Line.Size = UDim2New(0, Length, 0, SkeletonThickness)
                    Line.Rotation = math.deg(math.atan2(Delta.Y, Delta.X))
                end
                local TracerTarget = projectPart({"UpperTorso", "Torso", "HumanoidRootPart"})
                local PreviewSize = Items["Background"].Instance.AbsoluteSize
                local ThreeDVisible = BoxVisible and BoxOutline and BoxStyle == "ThreeD"
                local BoxThickness = MathClamp(MathFloor((tonumber(Settings.BoxThickness) or 1) + 0.5), 1, 3)
                local Front = {
                    Vector2New(PreviewSize.X * 0.24, PreviewSize.Y * 0.08),
                    Vector2New(PreviewSize.X * 0.76, PreviewSize.Y * 0.08),
                    Vector2New(PreviewSize.X * 0.76, PreviewSize.Y * 0.94),
                    Vector2New(PreviewSize.X * 0.24, PreviewSize.Y * 0.94),
                }
                local Back = {
                    Vector2New(PreviewSize.X * 0.29, PreviewSize.Y * 0.05),
                    Vector2New(PreviewSize.X * 0.81, PreviewSize.Y * 0.05),
                    Vector2New(PreviewSize.X * 0.81, PreviewSize.Y * 0.91),
                    Vector2New(PreviewSize.X * 0.29, PreviewSize.Y * 0.91),
                }
                local ThreeDPairs = {
                    {Front[1], Front[2]}, {Front[2], Front[3]}, {Front[3], Front[4]}, {Front[4], Front[1]},
                    {Back[1], Back[2]}, {Back[2], Back[3]}, {Back[3], Back[4]}, {Back[4], Back[1]},
                    {Front[1], Back[1]}, {Front[2], Back[2]}, {Front[3], Back[3]}, {Front[4], Back[4]},
                }
                for Index, Pair in ThreeDPairs do
                    local Line = ThreeDBoxSegments[Index]
                    local Delta = Pair[2] - Pair[1]
                    Line.Visible = ThreeDVisible
                    Line.BackgroundColor3 = BoxColor
                    Line.Position = UDim2New(0, (Pair[1].X + Pair[2].X) * 0.5, 0, (Pair[1].Y + Pair[2].Y) * 0.5)
                    Line.Size = UDim2New(0, Delta.Magnitude, 0, BoxThickness)
                    Line.Rotation = math.deg(math.atan2(Delta.Y, Delta.X))
                end
                local TracerOrigin = tostring(Settings.TracerOrigin or "Bottom")
                local TracerFrom
                if TracerOrigin == "Top" then
                    TracerFrom = Vector2New(PreviewSize.X * 0.5, 0)
                elseif TracerOrigin == "Center" then
                    TracerFrom = Vector2New(PreviewSize.X * 0.5, PreviewSize.Y * 0.5)
                elseif TracerOrigin == "Mouse" then
                    local MousePosition = UserInputService:GetMouseLocation() - Items["Background"].Instance.AbsolutePosition
                    TracerFrom = Vector2New(MathClamp(MousePosition.X, 0, PreviewSize.X), MathClamp(MousePosition.Y, 0, PreviewSize.Y))
                else
                    TracerFrom = Vector2New(PreviewSize.X * 0.5, PreviewSize.Y)
                end
                if ESPEnabled and Settings.TracerESP == true and TracerTarget then
                    local Delta = TracerTarget - TracerFrom
                    Items["Tracer"].Instance.Visible = true
                    Items["Tracer"].Instance.Position = UDim2New(0, (TracerFrom.X + TracerTarget.X) * 0.5, 0, (TracerFrom.Y + TracerTarget.Y) * 0.5)
                    Items["Tracer"].Instance.Size = UDim2New(0, Delta.Magnitude, 0, MathClamp(MathFloor((tonumber(Settings.TracerThickness) or 1) + 0.5), 1, 3))
                    Items["Tracer"].Instance.Rotation = math.deg(math.atan2(Delta.Y, Delta.X))
                else
                    Items["Tracer"].Instance.Visible = false
                end
                for Index, Pair in SkeletonPairs do
                    drawSkeletonLine(SkeletonSegments[Index], projectPart(Pair[1]), projectPart(Pair[2]))
                end
                for Original, Clone in RenderObjects do
                    if Original and Original.Parent and Clone and Clone.Parent then
                        local RelativeFrame = Root.CFrame:ToObjectSpace(Original.CFrame)
                        Clone.CFrame = RelativeFrame
                        ...
                        local Overlay = ChamsOverlays[Original]
                        if Overlay then
                            pcall(function()
                                Overlay.CFrame = RelativeFrame
                                Overlay.Size = Original.Size * 1.003
                                Overlay.Color = ChamsFillColor
                                Overlay.Material = Enum.Material.Neon
                                if ChamsEnabled and Original.Transparency < 1 then
                                    Overlay.Transparency = 1 - ((1 - Original.Transparency) * (1 - ChamsFillTransparency))
                                else
                                    Overlay.Transparency = 1
                                end
                            end)
            ...
            function Preview:Destroy()
                ...
                DisconnectModelConnections()
                ClearViewport()
                for Index, Callback in Library.WindowVisibilityCallbacks do
                    if Callback == ApplyVisibility then
                        table.remove(Library.WindowVisibilityCallbacks, Index)
                        break
                    end
                end
                Items["ESPPreview"]:Clean()
            end
        ...
        Library.Window = function(self, Data)
            ...
            local Items = { } do
                ...
                Items["UtilityPages"] = Instances:Create("Frame", {
                    Parent = Items["MainFrame"].Instance,
                    Name = "\0",
                    BorderColor3 = FromRGB(0, 0, 0),
                    AnchorPoint = Vector2New(0, 0),
                    Position = UDim2New(0.5, 8, 1, 8),
                    Size = UDim2New(0, 45, 0, 45),
                    ZIndex = 2,
                    Visible = false,
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(16, 18, 21)
                })  Items["UtilityPages"]:AddToTheme({BackgroundColor3 = "Background"})

                Instances:Create("UICorner", {
                    Parent = Items["UtilityPages"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(1, 0)
                })

                Items["UtilityHolder"] = Instances:Create("Frame", {
                    Parent = Items["UtilityPages"].Instance,
                    Name = "\0",
                    BackgroundTransparency = 1,
                    Size = UDim2New(1, 0, 1, 0),
                ...
                Instances:Create("UIPadding", {
                    Parent = Items["UtilityHolder"].Instance,
                    Name = "\0",
                ...
                local function UpdateUtilityPosition()
                    Items["UtilityPages"].Instance.Position = UDim2New(0.5, math.floor(Items["Pages"].Instance.AbsoluteSize.X / 2) + 8, 1, 8)
                end

                Items["Pages"]:Connect("Changed", function(Property)
                    if Property == "AbsoluteSize" then
                        UpdateUtilityPosition()
                    end
                end)
                task.defer(UpdateUtilityPosition)
            ...
            function Window:SetOpen(Bool)
                ...
                for _, Callback in Library.WindowVisibilityCallbacks do
                    pcall(Callback, Bool)
                end
            ...
            local OldSize = Items["MainFrame"].Instance.Size
            ...
            local function SetMinimized(Minimized)
                ...
                if IsMinimized then
                    ...
                    Items["MinimizeButton"].Instance.Visible = false
                    Items["UnMinimizeButton"].Instance.Visible = true
                    Items["Content"].Instance.Visible = false
                    Items["Pages"].Instance.Visible = false
                    Items["UtilityPages"].Instance.Visible = false
                    Items["MainFrame"]:Tween(nil, {Size = UDim2New(OldSize.X.Scale, OldSize.X.Offset, 0, 35)})
                else
                    Items["UnMinimizeButton"].Instance.Visible = false
                    Items["MinimizeButton"].Instance.Visible = true
                    Items["Content"].Instance.Visible = true
                    Items["Pages"].Instance.Visible = true
                    Items["UtilityPages"].Instance.Visible = Window.HasDetachedPages
        ...
        Library.Page = function(self, Data)
            ...
            local Page = {
                ...
                Detached = Data.Detached == true or Data.detached == true,
                TextOnly = Data.TextOnly == true or Data.textonly == true,
            ...
            local Items = { } do
                ...
                Items["Inactive"] = Instances:Create("TextButton", {
                    Parent = (Page.Detached and Page.Window.Items["UtilityHolder"] or Page.Window.Items["Holder"]).Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(0, 0, 0),
                    BorderColor3 = FromRGB(0, 0, 0),
                ...
                if Page.Detached then
                    Page.Window.HasDetachedPages = true
                    Page.Window.Items["UtilityPages"].Instance.Visible = true
                    Items["Inactive"].Instance.Size = UDim2New(0, 32, 0, 32)
                    Items["Inactive"].Instance.BackgroundTransparency = 0.35
                    Items["Text"].Instance.Visible = true
                    if Page.TextOnly then
                        Items["Icon"].Instance.Visible = false
                        Items["Text"].Instance.AnchorPoint = Vector2New(0.5, 0.5)
                        Items["Text"].Instance.Position = UDim2New(0.5, 0, 0.5, 0)
                        ...
                        Items["Text"].Instance.TextYAlignment = Enum.TextYAlignment.Center
                    else
            ...
            function Page:Switch(Bool)
                ...
                if Bool then
                    Items["Text"].Instance.Visible = true 
                    local TabWidth = if Page.Detached then 32 else math.max(32, math.ceil(Items["Text"].Instance.TextBounds.X) + (Page.TextOnly and 16 or 46))
                    Items["Inactive"]:Tween(nil, {BackgroundTransparency = 0, Size = UDim2New(0, TabWidth, 0, 32)})
                    if Page.TextOnly then
                    ...
                    Items["Text"].Instance.Visible = Page.Detached
                    local TabWidth = 32
                    Items["Inactive"]:Tween(nil, {BackgroundTransparency = Page.Detached and 0.35 or 1, Size = UDim2New(0, TabWidth, 0, 32)})
                    if Page.TextOnly then
        ...
        Library.Pages.SubPage = function(self, Data)
            ...
            function SubPage:Switch(Bool)
                ...
                if Bool then
                    Items["Text"].Instance.Visible = true 
                    local TabWidth = math.max(25, math.ceil(Items["Text"].Instance.TextBounds.X) + 46)
                    Items["Inactive"]:Tween(nil, {BackgroundTransparency = 0, Size = UDim2New(0, TabWidth, 0, 32)})
                    Items["Icon"]:ChangeItemTheme({ImageColor3 = "Accent"})
                    Items["Icon"]:Tween(nil, {ImageColor3 = Library.Theme.Accent, ImageTransparency = 0}) 
